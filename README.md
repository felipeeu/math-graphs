...
* [`src/math_graphs/`](src/math_graphs/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/math_graphs/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `math-graphs` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/math_graphs/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns math-graphs.example
  (:require [math-graphs.config :as config])
